...
export function playCelebrationSound() {
  ...
  try {
    ...
    function playExplosion(c: AudioContext, start: number) {
      const buffer = c.createBuffer(2, c.sampleRate * 0.5, c.sampleRate);
      for (let ch = 0; ch < 2; ch++) {
        const data = buffer.getChannelData(ch);
        for (let i = 0; i < buffer.length; i++) {
          const t = i / c.sampleRate;
          data[i] = (Math.random() * 2 - 1) * Math.exp(-t * 15) * (1 - t * 2);
        }
      }
      ...
      filter.frequency.exponentialRampToValueAtTime(150, start + 0.5);
      const gain = c.createGain();
    ...
    playExplosion(ctx, now + 0.2);

    const pitches = [880, 660, 440, 330, 220];
    pitches.forEach((freq, i) => {
      playCrackPop(ctx, now + 0.15 + i * 0.08, freq);
    });

    const whistleFreqs = [400, 700, 1000];
    whistleFreqs.forEach((freq, i) => {
      ...
      osc.frequency.exponentialRampToValueAtTime(freq * 0.4, now + 0.5 + i * 0.06);
      osc.type = "sine";
      gain.gain.setValueAtTime(0.06, now + 0.05 + i * 0.06);
      gain.gain.exponentialRampToValueAtTime(0.001, now + 0.5 + i * 0.06);
