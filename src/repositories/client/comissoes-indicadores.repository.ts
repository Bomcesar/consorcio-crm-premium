...
export async function getComissaoIndicador(id: string): Promise<ComissaoIndicador | null> {
  const user = await getAuthenticatedUser();
  const supabase = createClient();
  const { data, error } = await supabase.from("comissoes_indicadores")
    .select("*")
    .eq("id", id)
    .eq("usuario_id", user.id)
    .single();
...
export async function createComissaoIndicador(payload: ComissaoIndicadorInsert): Promise<ComissaoIndicador> {
  ...
  if (error || !data) throw new Error("Não foi possível salvar a comissão.");
  return data as ComissaoIndicador;
}

export async function updateComissaoIndicador(id: string, payload: ComissaoIndicadorUpdate): Promise<ComissaoIndicador> {
  const user = await getAuthenticatedUser();
  const supabase = createClient();
  const { data, error } = await supabase.from("comissoes_indicadores")
    .update(payload)
  ...
  if (error || !data) throw new Error("Não foi possível atualizar a comissão.");
  return data as ComissaoIndicador;
}
...
export async function deleteComissaoIndicador(id: string): Promise<void> {
  ...
  if (error) throw new Error("Não foi possível excluir a comissão.");
}
...
export async function getComissoesResumo(): Promise<{ total: number; totalPendente: number; totalPago: number; totalAReceber: number; quantidade: number }> {
  ...
  if (error) throw new Error("Não foi possível carregar o resumo de comissões.");
