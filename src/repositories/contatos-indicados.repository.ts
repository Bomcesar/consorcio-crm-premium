...
export async function getContatosIndicados(indicadorId: string): Promise<ContatoIndicado[]> {
  ...
  if (error) {
    throw new Error("Não foi possível carregar os contatos indicados.");
  }
...
export async function createContatoIndicado(payload: ContatoIndicadoInsert): Promise<ContatoIndicado> {
  ...
  if (error || !data) {
    throw new Error("Não foi possível salvar o contato indicado.");
  }

  return data as ContatoIndicado;
}

export async function updateContatoIndicado(
  id: string,
  payload: ContatoIndicadoUpdate
): Promise<ContatoIndicado> {
  const user = await getAuthenticatedUser();
  const supabase = await createClient();
  ...
  if (error || !data) {
    throw new Error("Não foi possível atualizar o contato indicado.");
  }
...
export async function deleteContatoIndicado(id: string): Promise<void> {
  ...
  if (error) {
    throw new Error("Não foi possível excluir o contato indicado.");
  }
}